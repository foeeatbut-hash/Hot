...
public sealed class MainForm : Form
{
    ...
    {
        new("Общее",      "Обзор",                  "", "Сводка проекта ОВИК"),
        new("Общее",      "Уровни",                 "", "Фильтр расчёта по этажам"),
        new("Общее",      "Карта",                  "", "Подсветка ролей и переназначение"),
        new("Общее",      "Классификатор",          "", "Роли объектов по типам"),
        new("Общее",      "Сопоставление",          "", "Откуда брать значения расчёта"),
        new("Общее",      "Исходные",               "", "Параметры расчёта и лимиты"),
        new("Отопление",  "Гидравлика",             "", "Расходы, потери, диаметры, кольца"),
        new("Отопление",  "Балансировка",           "", "Клапаны и преднастройки"),
        new("Отопление",  "Проверка модели",        "", "Что мешает расчёту и как исправить"),
        new("Вентиляция", "Аэродинамика",           "", "Потери давления в воздуховодах"),
        new("Вентиляция", "Воздухообмен",           "", "Кратности и расходы воздуха"),
    ...
    public MainForm(PluginContext ctx)
    {
        ...
        Text = $"RengaHeat — расчёты ОВИК (отопление и вентиляция) · {Build}";
        Width = 1080;
        Height = 720;
    ...
    private Control BuildHeader()
    {
        ...
        {
            Text = $"Расчёты ОВИК · отопление и вентиляция · {Build}", Dock = DockStyle.Top, Height = 18,
    ...
    private Control BuildSectionBody(string section) =>
        section switch
        {
            ...
            "Гидравлика" => BuildCalculation(),
            "Балансировка" => BuildBalancing(),
            "Аэродинамика" => BuildAerodynamics(),
            "Воздухообмен" => BuildAirExchange(),
    ...
    private Control BuildOverview()
    {
        ...
        if (_outcome is not null)
        {
            ...
            panel.Controls.Add(Card("Отопление · гидравлика",
                ...
                "Подробнее — разделы «Гидравлика», «Балансировка», «Проверка модели»."));
        }
        else
        {
            panel.Controls.Add(Card("Отопление · гидравлика",
                "Расчётный движок активен. Нажмите «Рассчитать» в шапке — получите расходы, потери, " +
                "критические кольца, балансировку и подбор диаметров.\r\n\r\n" +
                "Разделы: «Гидравлика», «Балансировка», «Проверка модели»."));
        }

        // Дисциплина «Вентиляция» — каркас; аэродинамический движок в разработке.
        var ventCount = _model.Objects.Values.Count(IsVentName);
        panel.Controls.Add(Card("Вентиляция · в разработке",
            "Аэродинамический расчёт воздуховодов (воздухообмен, потери давления, подбор сечений, " +
            "подбор вентилятора) — в разработке отдельным движком ядра. Числа пока не выдаются.\r\n" +
            $"Предварительно опознано вентиляционных объектов по имени: {ventCount}.\r\n\r\n" +
            "Разделы: «Аэродинамика», «Воздухообмен»."));
        return panel;
    }
    ...
    private static readonly (string Prefix, string Advice)[] FindingAdvice =
    {
        ...
        ("VEL-001", "Превышена скорость: примите рекомендованный Ду из «Гидравлики» → «Участки» или увеличьте диаметр в модели."),
        ("LOSS-001", "Превышены удельные потери: увеличьте диаметр участка (рекомендация — в «Гидравлике» → «Участки»)."),
        ("SIZE-002", "Диаметр участка не подходит под расход: см. колонку «Ду реком.» в «Гидравлике»."),
        ("BAL-001", "Балансировка: подберите/проверьте клапан и преднастройку в разделе «Балансировка»."),
        ("PUMP-001", "Насос не подобран: проверьте каталог насосов и требуемые расход/напор в «Гидравлике»."),
    ...
    {
        if (_outcome is null) return Info("Результаты появляются после расчёта. Нажмите «Рассчитать» в шапке окна.");
    ...
    // ---------- Вентиляция (каркас; аэродинамический движок в разработке) ----------

    // Ключевые слова имён/стилей вентиляционных объектов — предварительное распознавание сети
    // воздуховодов до появления аэродинамического движка (ролей вентиляции в ядре пока нет).
    private static readonly string[] VentKeywords =
    {
        "воздуховод", "воздухов", "duct", "решётк", "решетк", "grille", "диффузор", "diffuser",
        "анемостат", "вентилятор", " fan", "приточн", "вытяжн", "дефлектор", "зонт", "жалюзи",
        "дроссель", "шибер", "глушитель", "калорифер", "рекуператор", "вав", "vav",
    };

    private static bool IsVentName(NetworkObject o)
    {
        var n = ((o.Name ?? "") + " " + (o.StyleName ?? "")).ToLowerInvariant();
        foreach (var k in VentKeywords) if (n.Contains(k)) return true;
        return false;
    }

    private static string VentKindOf(NetworkObject o)
    {
        var n = ((o.Name ?? "") + " " + (o.StyleName ?? "")).ToLowerInvariant();
        if (n.Contains("воздуховод") || n.Contains("duct")) return "Воздуховоды";
        if (n.Contains("решётк") || n.Contains("решетк") || n.Contains("диффузор") ||
            n.Contains("анемостат") || n.Contains("grille") || n.Contains("diffuser")) return "Решётки/диффузоры";
        if (n.Contains("вентилятор") || n.Contains(" fan")) return "Вентиляторы";
        if (n.Contains("клапан") || n.Contains("дроссель") || n.Contains("шибер") || n.Contains("жалюзи")) return "Клапаны/дроссели";
        if (n.Contains("зонт") || n.Contains("дефлектор")) return "Зонты/дефлекторы";
        if (n.Contains("глушитель")) return "Шумоглушители";
        if (n.Contains("калорифер") || n.Contains("рекуператор")) return "Теплообмен/рекуперация";
        return "Прочее (вентиляция)";
    }

    /// <summary>Предварительная сводка вентиляционных объектов модели (по именам/стилям, не расчёт).</summary>
    private Control VentDetectedCard()
    {
        if (_model is null)
            return Info("Модель не загружена — распознавание вентиляционных объектов недоступно. " +
                        "Загрузите модель кнопками «Выделенное» или «Вся модель» в шапке.");
        var vent = _model.Objects.Values.Where(IsVentName).ToList();
        if (vent.Count == 0)
            return Card("Вентиляционные объекты",
                "В загруженной модели объекты с «вентиляционными» именами не распознаны.\r\n" +
                "Это предварительное распознавание по имени/стилю (ролей вентиляции в ядре пока нет). " +
                "Если сеть воздуховодов в модели есть — вернёмся к ней с появлением аэродинамического движка.");
        var byKind = vent.GroupBy(VentKindOf).OrderByDescending(g => g.Count())
            .Select(g => $"   • {g.Key}: {g.Count()}");
        return Card($"Вентиляционные объекты (распознано по имени: {vent.Count})",
            string.Join("\r\n", byKind) +
            "\r\n\r\nЭто предварительная сводка по именам/стилям, а не расчёт. Роли и стороны " +
            "вентиляции (приток/вытяжка) появятся вместе с аэродинамическим движком.");
    }

    private Control BuildAerodynamics()
    {
        var panel = new FlowLayoutPanel { Dock = DockStyle.Fill, FlowDirection = FlowDirection.TopDown, WrapContents = false, AutoScroll = true };
        panel.Controls.Add(Info(
            "Раздел в разработке. Аэродинамический расчёт воздуховодов пока НЕ выдаёт чисел — " +
            "чтобы не подменять обязательный расчёт неверными значениями. Ниже — что будет считаться " +
            "и что плагин уже видит в модели."));
        panel.Controls.Add(Card("Что будет считаться",
            "• Потери давления по участкам сети воздуховодов: трение (СП 60.13330) + местные сопротивления\r\n" +
            "• Скорости в сечениях и подбор размеров под лимиты (магистрали / ответвления / помещения)\r\n" +
            "• Располагаемое давление сети и критический путь (аналог критического кольца отопления)\r\n" +
            "• Требуемые параметры и подбор вентилятора (расход / полное давление)\r\n" +
            "• Увязка (балансировка) расходов по ветвям, подбор дроссель-клапанов\r\n\r\n" +
            "Движок переиспользует сетевой решатель отопления: среда — воздух (ρ≈1.2 кг/м³, ν≈15·10⁻⁶ м²/с), " +
            "поток — объёмный (м³/ч), сопротивление — трение по длине + КМС."));
        panel.Controls.Add(VentDetectedCard());
        return panel;
    }

    private Control BuildAirExchange()
    {
        var panel = new FlowLayoutPanel { Dock = DockStyle.Fill, FlowDirection = FlowDirection.TopDown, WrapContents = false, AutoScroll = true };
        panel.Controls.Add(Info(
            "Раздел в разработке. Расчёт воздухообмена (кратности, нормативные расходы приточного и " +
            "вытяжного воздуха по помещениям и системам) пока НЕ выдаёт чисел."));
        panel.Controls.Add(Card("Что будет считаться",
            "• Нормативный воздухообмен по помещениям: по кратности, по людям, по площади, по вредностям\r\n" +
            "• Баланс притока и вытяжки по помещениям, зонам и зданию\r\n" +
            "• Расчётные расходы на ветвях как исходные данные для аэродинамики\r\n" +
            "• Учёт назначения помещений и норм (СП 60.13330, ГОСТ 30494, отраслевые нормы)\r\n\r\n" +
            "Данные помещений берутся из модели Renga через «Сопоставление» (площадь, объём, назначение, " +
            "кратность) — параметризуемо, без жёстко зашитых имён свойств."));
        panel.Controls.Add(VentDetectedCard());
        return panel;
    }
    ...
    {
        var panel = new FlowLayoutPanel { Dock = DockStyle.Fill, FlowDirection = FlowDirection.TopDown, WrapContents = false, AutoScroll = true };
        panel.Controls.Add(Card("RengaHeat — расчёты ОВИК",
            "Параметрический расчёт инженерных систем ОВИК для Renga Professional: отопление и " +
            "вентиляция. Модель читается один раз и общая для дисциплин.\r\n\r\n" +
            $"Версия ядра: {CalculationSession.EngineVersion} · интерфейс: {Build}\r\n" +
            "Разработчик: Раупов Хусрав"));
        panel.Controls.Add(Card("Дисциплины",
            "ОТОПЛЕНИЕ — расчётный движок активен:\r\n" +
            ...
            "ВЕНТИЛЯЦИЯ — в разработке (отдельный аэродинамический движок):\r\n" +
            "   • Воздухообмен по помещениям и системам\r\n" +
            "   • Аэродинамика воздуховодов, подбор сечений и вентилятора\r\n" +
            "   Числа пока не выдаются — чтобы не подменять обязательный расчёт неверными значениями."));
        panel.Controls.Add(Card("Общее для дисциплин",
            ...
            "• Сопоставление: откуда брать каждое значение (параметризуемо, без зашитых имён)\r\n" +
