...
public static class Reports
{
    ...
    public static string SegmentsCsv(CalculationResult result)
    {
        ...
        sb.AppendLine("Участок;Система;Расход_кг/ч;Скорость_м/с;Re;Режим;Лимит_v;Удельные_Па/м;Лимит_R;" +
        ...
        foreach (var s in result.Segments)
            sb.AppendLine(string.Join(';',
                ...
                s.ReynoldsNumber > 0 ? F(s.ReynoldsNumber, 0) : "", s.FlowRegime,
    ...
    public static string SessionReport(SessionOutcome outcome)
    {
        ...
        sb.AppendLine($"\n{new string('=', 60)}");
        sb.AppendLine("ВНИМАНИЕ: перед выпуском документации проверьте результаты обязательным");
        sb.AppendLine("расчётом в специализированном ПО. Пакет сверки помогает подтвердить");
        sb.AppendLine("эквивалентность методик.");
        return sb.ToString();
    }
