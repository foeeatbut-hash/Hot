...
public sealed record RequirementsProfile
{
    ...
    /// <summary>
    /// Стандартный профиль по умолчанию: типовые лимиты систем отопления по СП.
    /// Все значения редактируются инженером в разделе «Исходные» — профиль не привязан
    /// к конкретному объекту или заказчику.
    /// </summary>
    public static RequirementsProfile Default() => new()
    {
        Name = "Стандарт",
        Version = "1",
        SourceDocument = "СП 60.13330, СП 73.13330 (типовые лимиты; редактируются в «Исходных»)",
    };

    /// <summary>
    /// Профиль ЧТУ «Новосаратовка» — пример профиля заказчика как данных (используется в тестах).
    /// Значения совпадают со стандартными по умолчанию.
    /// </summary>
